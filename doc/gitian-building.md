...
*Setup instructions for a gitian build of MillenniumPayX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MillenniumPayX
...
- [Building MillenniumPayX](#building-MillenniumPayX)
...
Clone the git repositories for MillenniumPayX and gitian and then checkout the MillenniumPayX version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/MillenniumPayX-crypto/MillenniumPayX.git
cd MillenniumPayX
...
Follow the instructions in [doc/release-process.md](release-process.md) in the MillenniumPayX repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building MillenniumPayX
----------------

To build MillenniumPayX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the MillenniumPayX repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/MillenniumPayX/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/MillenniumPayX-crypto/MillenniumPayX
...
For example:
```bash
URL=https://github.com/crowning-/MillenniumPayX.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit MillenniumPayX=${COMMIT} --url MillenniumPayX=${URL} ../MillenniumPayX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit MillenniumPayX=${COMMIT} --url MillenniumPayX=${URL} ../MillenniumPayX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit MillenniumPayX=${COMMIT} --url MillenniumPayX=${URL} ../MillenniumPayX/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/MillenniumPayX-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/MillenniumPayX-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/MillenniumPayX-build.assert
```
...
[MillenniumPayX/gitian.sigs](https://github.com/MillenniumPayX-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
